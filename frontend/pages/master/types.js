...
export default function TypesPage() {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      const items = Array.isArray(data) ? data : data.items || data.types || [];
      setRows(items);
  ...
  return (
    <MainLayout title="Types">
      <PageHeader
        title="Types"
        subtitle={`${total} type${total !== 1 ? 's' : ''} registered`}
        icon={<TuneIcon />}
        color="#4a148c"
        breadcrumbs={[{ label: 'Master Data' }, { label: 'Types' }]}
        actions={
      ...
      <AdminConfirmDialog
        ...
        title="Delete Type"
        description="Are you sure you want to delete this type? Enter admin password to confirm."
      />
    </MainLayout>
  );
}
