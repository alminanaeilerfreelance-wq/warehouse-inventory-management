...
const api = getCRUD('store-branches');
...
export default function StoreBranchesPage() {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      const items = Array.isArray(data) ? data : data.items || data['store-branches'] || data.storeBranches || [];
      setRows(items);
      setTotal(res.data.total || res.data.pagination?.total || items.length);
    } catch {
      enqueueSnackbar('Failed to load store branches', { variant: 'error' });
    } finally {
      setLoading(false);
    }
  ...
  const handleAdminConfirm = async () => {
    ...
    try {
      if (editId) {
        ...
        enqueueSnackbar('Store branch updated successfully', { variant: 'success' });
      } else {
        await api.create(formData);
        enqueueSnackbar('Store branch created successfully', { variant: 'success' });
      }
      setFormOpen(false);
      fetchData();
    } catch (err) {
  ...
  const handleDeleteConfirm = async () => {
    try {
      ...
      enqueueSnackbar('Store branch deleted', { variant: 'success' });
      setDeleteOpen(false);
      fetchData();
    } catch (err) {
  ...
  return (
    <MainLayout title="Store Branches">
      <PageHeader
        title="Store Branches"
        subtitle={`${total} branch${total !== 1 ? 'es' : ''} registered`}
        icon={<StoreIcon />}
        color="#00695c"
        breadcrumbs={[{ label: 'Master Data' }, { label: 'Store Branches' }]}
        actions={
      ...
      <FormDialog
        ...
        title={editId ? 'Edit Store Branch' : 'Add Store Branch'}
      ...
      <AdminConfirmDialog
        ...
        description="Are you sure you want to delete this store branch? Enter admin password to confirm."
      />
    </MainLayout>
  );
}
