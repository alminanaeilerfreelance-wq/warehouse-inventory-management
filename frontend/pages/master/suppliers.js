...
const api = getCRUD('suppliers');
const EMPTY_FORM = { name: '', email: '', contact: '', address: '', notes: '', status: 'Active' };
...
export default function SuppliersPage() {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      const items = Array.isArray(data) ? data : data.items || data.suppliers || [];
      setRows(items);
  ...
  const openEdit = (row) => {
    setFormData({ name: row.name || '', email: row.email || '', contact: row.contact || '', address: row.address || '', notes: row.notes || '', status: row.status || 'Active' });
  ...
  return (
    <MainLayout title="Suppliers">
      <PageHeader
        title="Suppliers"
        subtitle={`${total} supplier${total !== 1 ? 's' : ''} registered`}
      ...
      >
        <Stack spacing={2} mt={1}>
          ...
          <TextField label="Notes" value={formData.notes} onChange={(e) => setFormData((p) => ({ ...p, notes: e.target.value }))} fullWidth multiline rows={2} />
      ...
      <AdminConfirmDialog
        ...
        description="Are you sure you want to delete this supplier? Enter admin password to confirm."
      />
    </MainLayout>
  );
}
